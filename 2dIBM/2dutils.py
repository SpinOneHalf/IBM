...
def simulation(r, u, v, E, p, c,
    ...
    while t < tMax:
        ...
        FB = forceFluid(dx,dy,nx,ny,X0,dtheta)
...
def forceFluid(dx,dy,nx,ny,X0,dtheta):
    ...
    for i in range(nx):
        for j in range(ny):
            ...
            for k in range(p):
                xtemp=X0[k,:]
                d=np.sqrt((xtemp[0]-xp)**2+(xtemp[0]-yp)**2)
...
def generate_circile(r,n):
    xs=np.zeros((n,2))
    dtheta=(2*np.pi)/n
    for i in range(n):
        xs[i,:]=np.array([np.cos(dtheta*i),np.sin(dtheta*i)]*r)
    return xs,dtheta
